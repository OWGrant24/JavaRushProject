...
public class Level29Task10_Sportsman {
    ...
    public Level29Task10_Sportsman(int number) {
        this.number = number;
        chanceToHit = BigDecimal.valueOf(ThreadLocalRandom.current().nextDouble(Level29Task10_Constants.MIN_CHANCE_TO_HIT_IN_TARGET.doubleValue(), Level29Task10_Constants.MAX_CHANCE_TO_HIT_IN_TARGET.doubleValue()));
    }

    public void calculateFullTime() {
        resultTime = resultTime.add(BigDecimal.valueOf(countHitOutTarget * Level29Task10_Constants.PENALTY_TIME_PER_ONE_MISS));
        resultTime = resultTime.add(calculateTimeToRace());
    }

    private BigDecimal calculateTimeToRace() {
        return new BigDecimal(Level29Task10_Constants.RACE_LENGTH / averageSpeed.doubleValue() * 3600).setScale(2, BigDecimal.ROUND_HALF_UP);
    }

    public void calculateNumberOfMiss() {
        for (int i = 0; i < Level29Task10_Constants.NUMBER_OF_FIRE_LINES * 5; i++) {
